...
#define DECL_NLATTR_INTEGER_CHECK_(nla_data_name_, nla_data_type_, fmt_) \
	static inline void							\
	...
	{								\
		...
		for (size_t i = 0; i < ARRAY_SIZE(vecs); i++) {		\
			...
			TEST_NLATTR_(fd, nlh0, hdrlen,			\
				    ...
				    nla_type, nla_type_str,		\
				    sizeof(vecs[i]) + 1,		\
				    vecs + i, sizeof(vecs[i]),		\
				    printf(fmt_, vecs[i]));		\
		}							\
	}

DECL_NLATTR_INTEGER_CHECK_(u32, uint32_t, "%u")
DECL_NLATTR_INTEGER_CHECK_(x32, uint32_t, "%#x")
