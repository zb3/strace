...
main(int argc, char **argv)
{
	...
	struct seccomp_notif *notif = tail_alloc(sizeof(*notif));
...
# endif
	...
	struct seccomp_notif_resp *resp = tail_alloc(sizeof(*resp));
	...
	pidns_print_leader();
	printf("ioctl(-1, " XLAT_FMT ", {id=0xdeadc0debadc0ded"
	       ", val=-2675229516524167443, error=" XLAT_KNOWN(-63, "-ENOSR")
	...
	static const struct {
		...
		{ IOC_ANOTHER_DIR(SECCOMP_IOCTL_NOTIF_ID_VALID, _IOC_READ),
		  NVERB("_IOC(_IOC_READ, 0x21, 0x2, 0x8) /* ")
		  "SECCOMP_IOCTL_NOTIF_ID_VALID_WRONG_DIR" NVERB(" */") },
	};
	uint64_t *id = tail_alloc(sizeof(*id));
	...
	struct seccomp_notif_addfd *addfd = tail_alloc(sizeof(*addfd));
